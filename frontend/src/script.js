...
document.getElementById("fileInput").addEventListener("change", async () => {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Server error during File Uploading");
...
// Extract relevant features for graph type
featureSelectionBtn.addEventListener("click", async () =>{
    ...
    try {
        ...
        if (!response.ok) throw new Error("Server error during Feature Selection");
        ...
        `;
        window.featuresSelected = {
            X_axis: xAxisFeatures[0] || null,
            Y_axis: yAxisFeatures[0] || null,
            Group: null
        }

        document.getElementById("xAxisSelect").addEventListener("change", (e) => {
            window.featuresSelected.X_axis = e.target.value;
        })
        document.getElementById("yAxisSelect").addEventListener("change", (e) => {
            window.featuresSelected.Y_axis = e.target.value;
        })
        document.getElementById("groupSelect").addEventListener("change", (e) => {
            window.featuresSelected.Group = e.target.value;
        })

    }
    catch (err) {
        console.error(err);
        alert("❌ Failed to Select Features. Check console for details.");
    }
})
...
generateChartBtn.addEventListener("click", async () =>{
    if (!featuresSelected) {
        alert("Select The features for your axes first!");
        return;
    }
    const chartType = document.getElementById("chartType").value;
    const formData = new FormData();
    ...
    formData.append("X_axis", window.featuresSelected.X_axis);
    formData.append("Y_axis", window.featuresSelected.Y_axis);
    formData.append("Group", window.featuresSelected.Group);
    try {
        const response = await fetch(`${API_BASE_URL}/analysis`, {
            method: "POST",
            body: formData
        })
        if (!response) throw new Error("Server error during Graph Generation");
        ...
        Plotly.newPlot("chartDisplay", data.Graph.data, data.Graph.layout);
    }
    catch (err) {
        console.error(err);
